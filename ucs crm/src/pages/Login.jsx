...
export default function Login({ onLogin }) {
  ...
  if (showSignup) {
    return (
      <div className="login-page">
        <WhatsAppSignup
          onBack={() => { setShowSignup(false); setShowWa(false) }}
          onSwitchToLogin={() => { setShowSignup(false); setShowWa(true) }}
        />
      </div>
    )
  }
  ...
  if (showWa) {
    return (
      <div className="login-page">
        <WhatsAppLogin onBack={() => setShowWa(false)} onSwitchToSignup={() => setShowSignup(true)} />
      </div>
    )
  }
        ...
        <div style={{ marginTop: 16, textAlign: 'center' }}>
          <button onClick={() => setShowWa(true)}
            style={{
              ...
              color: '#25D366', cursor: 'pointer', display: 'inline-flex',
