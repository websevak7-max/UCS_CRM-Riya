...
export default function WhatsAppSignup({ onBack, onSwitchToLogin }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!firstName || !lastName || !orgName || !email || !password) return
    setLoading(true)
    setError('')
    setSuccess('')
    try {
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <div style={{ fontSize: 20, fontWeight: 700, color: '#1f2937' }}>Create Account</div>
        <div style={{ fontSize: 13, color: '#6b7280', marginTop: 4 }}>Get started with WhatsApp CRM</div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Organization Name</label>
        ...
        {error && (
          <div style={{ fontSize: 12, color: '#dc2626', marginBottom: 12, padding: '8px 10px', background: '#fef2f2', borderRadius: 6 }}>{error}</div>
        )}
        {success && (
          <div style={{ fontSize: 12, color: '#059669', marginBottom: 12, padding: '8px 10px', background: '#f0fdf4', borderRadius: 6 }}>{success}</div>
        )}

        <button type="submit" disabled={loading || !firstName || !lastName || !orgName || !email || !password}
          style={{
